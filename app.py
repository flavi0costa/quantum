...
st.caption("🚀 App ULTIMATE por Grok • Nova aba de Padrões de Candles adicionada • Apenas educativo • Não é aconselhamento financeiro")import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import plotly.graph_objects as go
...
if st.button("Analisar Ticker Manual", type="primary") and manual_ticker:
    with st.spinner(f"A analisar {manual_ticker.upper()}..."):
        ...
        if manual_df is not None:
            latest = manual_df.iloc[-1]
            signal_text, _ = generate_signal(manual_df)
            st.success(f"✅ Análise completa de **{manual_ticker.upper()}**")

            col1, col2, col3, col4 = st.columns(4)
            col1.metric("Preço", f"${latest['Close']:.2f}", f"{(latest['Close']/manual_df.iloc[-2]['Close']-1)*100:+.2f}%")
            col2.metric("Sinal", signal_text)
            col3.metric("ATR", f"{latest.get('ATR',0):.2f}")
            col4.metric("Stop 2×ATR", f"${latest['Close'] - 2*latest.get('ATR',0):.2f}")

            # Mostra as abas para o ticker manual
            ...
            # Exemplo da primeira aba (podes copiar as outras abas do teu código e mudar df para manual_df)
            with tabs_manual[0]:
                fig = make_subplots(specs=[[{"secondary_y": True}]])
            ...
            # Aba de Padrões de Candles para o manual
            with tabs_manual[12]:
            ...
            st.error(f"Não foi possível carregar dados para **{manual_ticker.upper()}**")
    with tabs[0]:
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    fig.add_trace(go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'], name="OHLC"))
    fig.add_trace(go.Scatter(x=df.index, y=df['SMA50'], name="SMA50", line=dict(color="orange")))
